    ...
    handleSearch(e){
     ...
     getHighlighted = (string, key = this.state.searchKey.toLowerCase().trim()) => {
        if( typeof string === 'string'){
            ...
            return(
                <span key={string + key + cuid()} className="highlighted">{string.substring(0,startIndex)}<i>{string.substring(startIndex,endIndex)}</i>{string.substring(endIndex,string.length)}</span>
            )
        }else{
            <span>{string}</span>
        }
                            ...
                            matchedEvents.length > 0 ? (
                                <ul className="event-list">
                                    ...
                                    {matchedEvents.map((event) =>{
                                        ...
                                        return ( <li key={cuid()} onClick={()=>{this.props.actions.setSelectedDay(momentDay);this.props.actions.updateDisplayedDate(momentDay)}}>
